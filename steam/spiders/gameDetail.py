...
class GamedetailSpider(Spider):
    ...
    def parse_detail(self, response):
        ...
        if(response.xpath('//*[@id="game_area_purchase"]//div[contains(@class,"final_price")]/text()').extract()!=[]):
            item['price'] = \
                response.xpath('//*[@id="game_area_purchase"]//div[contains(@class,"final_price")]/text()').extract()[
                    0].strip().strip("¥ ")
        else:
            item['price'] = \
        ...
        if (response.xpath(
            ...
            item['review'] = \
                response.xpath('//*[@id="game_highlights"]//span[contains(@class,"nonresponsive_hidden")]').extract()[0] \
                    .replace("\t", "").replace("\r\n", "")
